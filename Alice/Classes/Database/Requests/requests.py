...
class Requests :
    ...
    @classmethod
    def update_favorite_station(cls, title, user_id, station_code):
        # pour modifier une station favori
        db = Database()
        sql = "UPDATE favorites SET title = %s WHERE id_user = %s AND station_code = %s"
        params = (title, user_id, station_code)
        db.mutate_and_close(sql, params)
